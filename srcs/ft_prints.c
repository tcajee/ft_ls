...
/*   Updated: 2019/08/05 13:40:24 by tcajee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_rec_prints(char *path, t_flags *flags)
{
	...
	while ((s_dirent = readdir(dir)) != NULL)
	{
		...
		/* if (ft_ls_isdir(t_strjoin(ft_strjoin(path, "/"), s_dirent->d_name))) */
		
		if (ft_ls_isdir(s_dirent->d_name))
		{
			if (s_dirent->d_name[0] == '.' && s_dirent->d_name[1] == '\0')
				continue ;
			if (s_dirent->d_name[0] == '.' && s_dirent->d_name[1] == '.' && s_dirent->d_name[2] == '\0')
				continue ;
			if (s_dirent->d_name[0] == '.' && (s_dirent->d_name[1] != '.' || s_dirent->d_name[1] != '/') && *flags & F_a)
			{
